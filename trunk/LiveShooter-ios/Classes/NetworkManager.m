...
//    assert(str != nil);
    ...
    trimmedStr = [str stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]; //去除空格？
...
- (NSString *)pathForTestImage:(NSUInteger)imageNumber //这个函数改变了图片的大小，编号越往后的越大。
    ...
    // again.
{
    ...
    attrs = [fileManager attributesOfItemAtPath:originalFilePath error:NULL];  //获取文件信息 包括 创建时间，修改时间什么的。
    assert(attrs != nil);
