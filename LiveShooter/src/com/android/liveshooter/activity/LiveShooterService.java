...
public class LiveShooterService {
	...
	private final String serverUrl = "http://192.168.1.143";
	...
	public void updateVideoRecording() {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					Log.i("exception", e.getMessage()+" update ");
				}
			}
		}.start();
	}
	...
	private void genSegment(InputStream in) throws Exception {
		...
		boolean isSuccess = uploadFile("192.168.1.143", "21", "live","shooter", "", segmentName + ".mp4", in);
		...
		Log.i("msg", "finalResult " +count +" = "+ finalResult);
	}
	
	private void updateSegment(InputStream in)throws Exception{
		
		XmlRpcClient client = new XmlRpcClient(serverUrl + ":8000/");
		count++;
		boolean isSuccess = uploadFile("192.168.1.143", "21", "live","shooter", "", segmentName+"_"+count+ ".mp4", in);
		Log.i("msg", "isSuccess = " + isSuccess);

		Vector<Object> paramsDemo = new Vector<Object>();
		paramsDemo.addElement(segmentName);
		paramsDemo.add(count+"");
		String finalResult = client.execute("updateSegment", paramsDemo).toString();
		Log.i("msg", "finalResult " +count +" = "+ finalResult);
	}
	...
	public boolean uploadFile(String url, String port, String username, String password, String path, String filename, InputStream input) {
		...
		try {
			...
			if ("21".equals(port)) {
				...
				ftp.connect("live", portNO);// 连接FTP服务器
			}
